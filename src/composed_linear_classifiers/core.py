"""Research code for composed nonlinear classifiers built from linear models.

The module is derived from `notebooks/main.ipynb` and keeps the experimental
protocol used in the thesis: minority class is always encoded as the positive
class, evaluation uses stratified k-fold CV with a fixed random state, and
Optuna tuning maximizes F1 for the minority class.
"""
...
from sklearn.datasets import fetch_openml, load_breast_cancer as _load_breast_cancer
...
def load_secom() -> BinaryDataset:
    """SECOM: defect detection; defect (~6.6%) is the positive class.

    OpenML dataset 43587 has no default target attribute, therefore the
    target must be read explicitly from the `Pass/Fail` column.
    """
    ds = fetch_openml(data_id=43587, as_frame=True, parser="auto")
    if ds.frame is None or "Pass/Fail" not in ds.frame.columns:
        raise ValueError("OpenML 43587 must contain a Pass/Fail target column")
    df = ds.frame.copy()
    y = _encode_target(df["Pass/Fail"])
...
DATASET_LOADERS: dict[str, Callable[[], BinaryDataset]] = {
    "breast_cancer": load_breast_cancer,
    "bank_marketing": load_bank_marketing,
    "adult": load_adult,
    "credit_card_fraud": load_credit_card_fraud,
    "secom": load_secom,
    "secom_selected": load_secom_selected,
}
...
class BaselineMLP(ComposedClassifier):
    ...
    def _suggest_params(self, trial) -> dict:
        ...
        return {
            "hidden_layer_sizes": literal_eval(arch),
...
# Базовые модели (4 линейных + 5 нелинейных) + 35 составных конфигураций
LINEAR_BASELINES: list = [
...
def describe_model_registry(registry: list = MODEL_REGISTRY) -> dict[str, int]:
    """Return registry sizes without printing during module import."""
    return {
        "total": len(registry),
        "linear_baselines": len(LINEAR_BASELINES),
        "nonlinear_baselines": len(NONLINEAR_BASELINES),
        "composed_models": (
            len(registry) - len(LINEAR_BASELINES) - len(NONLINEAR_BASELINES)
        ),
    }


def model_name(model_or_class) -> str:
    """Stable display name for a model class or instance."""
    if isinstance(model_or_class, type):
        return model_or_class().__class__.__name__
    return type(model_or_class).__name__


def select_model_classes(names: list[str] | None = None) -> list[type]:
    """Select model classes by display name; return full registry if names is None."""
    if names is None:
        return list(MODEL_REGISTRY)
    available = {model_name(cls): cls for cls in MODEL_REGISTRY}
    missing = sorted(set(names) - set(available))
    if missing:
        raise ValueError(f"Unknown model names: {missing}. Available: {sorted(available)}")
